...
def _raw_metrics(cluster: ClusterObservation | None) -> dict[str, object]:
    if cluster is None:
        return {"detected": False}
    values = cluster.to_dict()
    values.pop("particle_ids", None)
    values.pop("history", None)
    distribution = values.pop("species_distribution", [])
    for index, value in enumerate(distribution):
        values[f"species_{index}_fraction"] = value
    return values
...
def run_validation() -> list[ValidationResult]:
    ...
    results.append(ValidationResult("A. Compact translating blob", {"Cohesion": compact.cohesion_score if compact else 0.0, "Structure": compact.structure_score if compact else 0.0}, bool(compact and compact.cohesion_score >= 0.65), "compact local graph", "detected with high cohesion and coherent translation", "detected; cohesion remains high" if compact else "not detected", _raw_metrics(compact)))
    ...
    results.append(ValidationResult("B. Two blobs with sparse bridge", {"Structure": bridge_score, "Bridge Fraction": bridged_cluster.bridge_fraction if bridged_cluster else 0.0, "Compactness": bridged_cluster.compactness if bridged_cluster else 0.0}, bool(bridged_cluster and bridge_score < 0.65), "sparse bridge is penalized", "detected, with bridge fraction and structure score penalizing the weak connection", "bridge penalty observed" if bridged_cluster else "not detected", _raw_metrics(bridged_cluster)))
    ...
    results.append(ValidationResult("C. Uniform random cloud", {"Structure": random_score}, random_score < 0.65, "no persistent cohesive component", "no primary-quality cohesive structure", "low structure score / no qualifying cohesive component", _raw_metrics(random_cluster)))
    ...
    results.append(ValidationResult("D. Expanding cloud", {"Structure": expanding.structure_score if expanding else 0.0, "Rg CV": expanding.rg_cv if expanding else 0.0, "Shape": expanding.shape_score if expanding else 0.0}, bool(expanding and (expanding.shape_score < 0.7 or expanding.structure_score < 0.65)), "growing Rg lowers temporal shape score", "temporal shape instability lowers the score", "shape instability observed" if expanding else "not tracked", _raw_metrics(expanding)))
    ...
    results.append(ValidationResult("E. Stationary crystal (Frozen crystal)", {"Cohesion": crystal.cohesion_score if crystal else 0.0, "Dynamics": crystal.dynamic_score if crystal else 0.0}, bool(crystal and crystal.cohesion_score >= 0.65 and crystal.dynamic_score <= 0.1), "cohesive but frozen", "high cohesion with low dynamics", "cohesive and low-dynamics as expected" if crystal else "not detected", _raw_metrics(crystal)))
    ...
    results.append(ValidationResult("F. Coherent moving structure with internal circulation", {"Structure": dynamic.structure_score if dynamic else 0.0, "Dynamics": dynamic.dynamic_score if dynamic else 0.0}, bool(dynamic and dynamic.structure_score >= 0.55 and dynamic.dynamic_score >= 0.5), "nonzero internal circulation", "structure remains cohesive while internal motion is measurable", "cohesive high-dynamics structure observed" if dynamic else "not tracked", _raw_metrics(dynamic)))
    ...
    results.append(ValidationResult("G. Two structures crossing", {"Identity": identity, "Tracked": float(len(tracker.active))}, identity >= 0.40, "particle IDs preserved during a pass", "two identities remain separable through the crossing", f"{len(tracker.active)} tracked structures; identity={identity:.2f}", {"tracked_cluster_count": len(tracker.active), "identity_score": identity}))
    ...
    toroidal_cluster = toroidal_observations[-1] if toroidal_observations else None
    results.append(ValidationResult("H. Structure crossing toroidal boundary (Toroidal boundary crossing)", {"Identity": toroidal_identity, "Centroid X": toroidal_centroid, "Rg": toroidal_cluster.radius_of_gyration if toroidal_cluster else 0.0}, toroidal_identity >= 0.40 and (toroidal_centroid < 5 or toroidal_centroid > 195), "minimum-image centroid and identity", "identity and compactness survive a world-wrap crossing", f"centroid_x={toroidal_centroid:.2f}; identity={toroidal_identity:.2f}", _raw_metrics(toroidal_cluster)))
